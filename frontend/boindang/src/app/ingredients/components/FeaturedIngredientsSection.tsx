  ...
  return (
    ...
    >
      <div className="relative w-full h-40">
        ...
        {ingredient.tag && (
          <span className={`absolute top-2 right-2 text-xs font-semibold px-2 py-1 rounded-md ${
            theme === 'bad' ? 'bg-yellow-100 text-yellow-700' : 'bg-purple-100 text-purple-700'
          }`}>
            {ingredient.tag}
          </span>
        )}
...
const FeaturedIngredientsCarousel: React.FC<FeaturedIngredientsSectionProps> = ({ title, ingredients, theme }) => {
  ...
  return (
    <section className="mb-6">
      ...
      <div className="flex overflow-x-auto space-x-4 pb-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100"> {/* 가로 스크롤 및 스크롤바 스타일 */} 
