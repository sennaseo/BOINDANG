    ...
    <div className="flex flex-col min-h-screen bg-background text-text-primary">
      ...
      <section className="p-4 flex items-center">
        <div className="relative w-16 h-16 rounded-full overflow-hidden mr-4 border border-gray-200 flex-shrink-0 bg-gray-200">
          {/* 프로필 이미지 - next/image 사용 (임시 주석 처리) */}
          {/* 
          <Image
      ...
      <nav className="flex border-b border-gray-200">
        <button
          ...
          className={`flex-1 py-3 text-center font-semibold transition-colors duration-200 ease-in-out ${activeTab === 'posts' ? 'text-[#6C2FF2] border-b-2 border-[#6C2FF2]' : 'text-gray-500 hover:text-gray-700'}`}
        >
          작성글
        </button>
        <button
          onClick={() => setActiveTab('comments')}
          className={`flex-1 py-3 text-center font-semibold transition-colors duration-200 ease-in-out ${activeTab === 'comments' ? 'text-[#6C2FF2] border-b-2 border-[#6C2FF2]' : 'text-gray-500 hover:text-gray-700'}`}
        >
          작성댓글
        </button>
      </nav>
